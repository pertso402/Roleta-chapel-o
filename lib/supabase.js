...
export async function clienteDaSessao(sessaoId) {
  ...
  return c?.telefone ? c : null;
}

export async function resgatar({ sessaoId, nome, telefone, consentimento, origem, utmSource, utmCampaign, lote }) {
  // Campanha: os dados vêm do disparo, não do formulário. Só cai aqui quando a
  // sessão tem ref — QR de adesivo continua exigindo o preenchimento, porque aí
  // a casa realmente não sabe quem é.
  if (!telefone || !nome) {
